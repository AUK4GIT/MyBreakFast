...
enum {
    ...
    ProfileGetUserProfileInfoReqId=13,
    ProfileUpdateProfileInfoReqId,
    ProfileGetDefaultPaymentOptionReqId,
};
...
- (void)requestProfileInformationWithCompletionHandler:(ASGetProfileInfoCallBack)callback{
    ...
    if (oauthStatus.error != nil) {
        oauthStatus = [CTSOauthManager fetchPasswordSigninTokenStatus:keystore];
    ...
    CTSRestCoreRequest* request = [[CTSRestCoreRequest alloc]
                                   initWithPath:MLC_PROFILE_GET_PROFILE_PATH
                                   requestId:-1
                                   headers:[CTSUtility readOauthTokenAsHeader:oauthToken]
                                   parameters:nil
                                   json:nil
    ...
    [restCore requestAsyncServer:request
                      completion:^(CTSRestCoreResponse *response) {
                          [self handleGetUserProfile:response
                                            callback:callback];
...
- (void)requestUpdateProfileInformation:(CTSProfileUpdate *)profileInfo
    ...
    if (![CTSUtility validateMobile:profileInfo.mobile]) {
        [self updateProfileHelper:[CTSError getErrorForCode:MobileNotValid] callback:callback];
        return;
    }
    
//    profileInfo.email = nil;
//    profileInfo.mobile = nil;
    
    NSString *profileUpdatePath = [NSString stringWithFormat:@"%@%@",[CTSUtility fetchFromEnvironment:BASE_URL],MLC_PROFILE_UPDATE_PROFILE_PATH];
    
    CTSRestCoreRequest* request = [[CTSRestCoreRequest alloc]
                                   initWithPath:profileUpdatePath
                                   requestId:ProfileUpdateProfileInfoReqId
                                   headers:[CTSUtility readOauthTokenAsHeader:oauthToken]
                                   parameters:nil
                                   json:[profileInfo toJSONString]
                                   httpMethod:PUT];
    request.isAlternatePath = YES;
    
    [restCore requestAsyncServer:request
                      completion:^(CTSRestCoreResponse *response) {
                          [self handleUpdateUserProfile:response
                                            callback:callback];
...
-(void)requestSaveCard:(CTSPaymentDetailUpdate*)paymentInfo completion:(ASSaveCardsCallback )callback{
    ...
    if (oauthStatus.error != nil || oauthToken == nil) {
        callback(nil,oauthStatus.error);
        return;
    } else {
        
        CTSErrorCode error = [paymentInfo validate];
        if (error != NoError) {
            callback(nil,[CTSError getErrorForCode:error]);
            return;
        }
    }
    ...
    CTSRestCoreRequest* request = [[CTSRestCoreRequest alloc]
                                   initWithPath:MLC_PROFILE_SAVE_CARD_PATH
                                   requestId:-1
                                   headers:[CTSUtility readOauthTokenAsHeader:oauthToken]
                                   parameters:nil
                                   json:[paymentInfo toJSONString]
    ...
    [restCore requestAsyncServer:request completion:^(CTSRestCoreResponse *response) {
        ...
        CTSSaveCardResponse* saveCardResp = nil;
        NSError *error = response.error;
        
        if (error == nil) {
            saveCardResp =
            [[CTSSaveCardResponse alloc] initWithString:response.responseString
                                                   error:&jsonError];
...
- (void)handleGetUserProfile:(CTSRestCoreResponse*)response callback:(ASGetProfileInfoCallBack)callback {
    
    NSError* error = response.error;
    JSONModelError* jsonError;
    CTSUserProfile* updateProfile = nil;
    if(error == nil){
        updateProfile = [[CTSUserProfile alloc] initWithString:response.responseString error:&jsonError];
    }
    [self getProfileHelper:updateProfile error:error callback:callback];
    
}

- (void)handleUpdateUserProfile:(CTSRestCoreResponse*)response callback:(ASUpdateProfileInfoCallBack)callback{
    
    [self updateProfileHelper:response.error callback:callback];
...
-(void)updateProfileHelper:(NSError *)error callback:(ASUpdateProfileInfoCallBack)callback{
    if (callback != nil) {
        callback(error);
    }
}

-(void)getProfileHelper:(CTSUserProfile *)userProfile
                  error:(NSError *)error
               callback:(ASGetProfileInfoCallBack)callback{
