...
#define UserLoginIdKey @"UserLoginIdKey"
+(void)storeLoginId:(NSString *)string{
    LogTrace(@"Storing Login Id %@",string);
    [self saveToDisk:string as:UserLoginIdKey];

}
+(NSString *)getLoginId{
    return [self readDataFromDisk:UserLoginIdKey];

}

+(void)removeLoginId{
    [self removeFromDisk:UserLoginIdKey];
}
