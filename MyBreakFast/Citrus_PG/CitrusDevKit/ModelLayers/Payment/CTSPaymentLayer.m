...
#define TempCVVKey @"tempCVV"
#define TempCVVSignatureKey @"tempCVVSignature"
#import "HVDOverlay.h"
...
- (void)requestChargePayment:(CTSPaymentDetailUpdate*)paymentInfo
    ...
    if([CitrusPaymentSDK isOneTapPaymentEnabled] == YES){
        CTSCVVEncryptor *encryptor = [[CTSCVVEncryptor alloc] initWithDefaultLoginId];
        NSString *cvvString = [encryptor getCVVForSignature: paymentOption.token];
        if(cvvString == nil && paymentOption.cvv != nil){
            LogTrace(@"no oneTapPaymentInfo found for the token %@",paymentOption.token);
            tempCVVStoreObject = paymentInfo;
        }
        else{
            LogTrace(@"found Stored OneTap Paymentinfo %@, will do the OneTapPayment",cvvString);
            tempCVVStoreObject = nil;
            paymentOption.cvv = cvvString;
    ...
    //check if its tokenized card payment then save the token against the cvv
...
-(BOOL)isCVVPresentForToken:(NSString *)token{
    CTSCVVEncryptor *encryptor = [[CTSCVVEncryptor alloc] initWithDefaultLoginId];
    return [encryptor isCVVStored:token];
}



//-(void)tempSaveCVV:(NSString *)cvv token:(NSString *)token{
//    [CTSUtility saveToDisk:token as:TempCVVSignatureKey];
//    [CTSUtility saveToDisk:cvv as:TempCVVKey];
//}
//
//-(void)resetTempCVV{
//    [CTSUtility removeFromDisk:TempCVVSignatureKey];
//    [CTSUtility removeFromDisk:TempCVVKey];
//}
//
//-(NSString *)getTempCVV{
//    return [CTSUtility readDataFromDisk:TempCVVKey];
//}
//
//-(NSString *)getTempCVVSignature{
//    return [CTSUtility readDataFromDisk:TempCVVKey];
//}
...
- (void)requestLoadMoneyInCitrusPay:(CTSPaymentDetailUpdate *)paymentInfo
    ...
    if([CitrusPaymentSDK isOneTapPaymentEnabled] == YES){
        ...
        NSString *cvvString = [encryptor getCVVForSignature: paymentOption.token];
        if(cvvString == nil && paymentOption.cvv != nil){
            LogTrace(@"no stored one tap payment info found for the token %@",paymentOption.token);
            tempCVVStoreObject = paymentInfo;
        }
        else{
            LogTrace(@"found stored one tap payment info %@, will do the one tap payment ",cvvString);
            tempCVVStoreObject = nil;
            paymentOption.cvv = cvvString;
        }
    }
    ...
    __block HVDOverlay *overlay;
     overlay = [self displayOverlay:overlay];
    ...
    [self requestLoadMoneyInCitrusPay:paymentInfo withContact:contactInfo withAddress:userAddress amount:amount returnUrl:returnUrl  customParams:custParams withCompletionHandler:^(CTSPaymentTransactionRes *paymentInfo, NSError *error) {
        if([CitrusPaymentSDK isLoaderEnabled]){
            [overlay dismiss];
        }
        if(!error){
                     BOOL hasSuccess =
...
-(HVDOverlay *)displayOverlay:(HVDOverlay *)overlay{
    if([CitrusPaymentSDK isLoaderEnabled]){
        UIColor* overlayColor = [CitrusPaymentSDK getLoaderColor];
        overlay = [HVDOverlay spinnerOverlay];
        if(overlayColor){
            [overlay setTintColor:overlayColor];
        }
        [overlay show];
    }
    return overlay;
...
-(void)requestGetPGHealthWithCompletionHandler:(ASGetPGHealth)callback{
    ...
    CTSRestCoreRequest* request = [[CTSRestCoreRequest alloc]
                                   initWithPath:[NSString stringWithFormat:@"/utility/%@/pgHealth", [CTSUtility fetchVanityUrl:[CTSUtility keyStore]]]
...
-(void)cvvEncrypt:(CTSCitrusCashRes *)response error:(NSError *)error{
    if(tempCVVStoreObject != nil && error == nil && response != nil && [CitrusPaymentSDK isOneTapPaymentEnabled] == YES){
        
        CTSPaymentOption *option = [tempCVVStoreObject.paymentOptions objectAtIndex:0];
        __block NSString *token = nil;
        __block NSString *cvv = option.cvv;
        
        if(option.token == nil){
            CTSProfileLayer *profile = [CitrusPaymentSDK fetchSharedProfileLayer];
            LogTrace(@"profile layer %@",profile);
            [profile requestSaveCard:tempCVVStoreObject completion:^(CTSSaveCardResponse *saveCardreponse, NSError *error) {
                LogTrace(@"saved the new card for one tap payment info %@",saveCardreponse.fingerPrint);
                LogTrace(@"error %@",error);

                token = saveCardreponse.fingerPrint;
                if(token != nil){
                    CTSCVVEncryptor *encryptor = [[CTSCVVEncryptor alloc ] initWithDefaultLoginId];
                    [encryptor storeCVV:cvv signature:token];
                    LogTrace(@"stored one tap payment info %@ for token %@",cvv,token);
                    LogTrace(@"one tap payment info stored for token %d",[encryptor isCVVStored:token]);
                }
            ...
            if(token != nil){
                CTSCVVEncryptor *encryptor = [[CTSCVVEncryptor alloc ] initWithDefaultLoginId];
                [encryptor storeCVV:cvv signature:token];
                LogTrace(@"one tap payment info %@ for token %@",cvv,token);
                LogTrace(@"is one tap payment info for token %d",[encryptor isCVVStored:token]);
            }
...
//Vikas change No -> Yes
    if(citrusCashBackViewController.navigationController){
        if([CitrusPaymentSDK isPushEnabled] == YES){
            [citrusCashBackViewController.navigationController pushViewController:paymentWebViewController animated:YES];
        }
        else{
...
-(void)dismissController{
    if([CitrusPaymentSDK isPushEnabled]){
