           ...
           completionHandler:(ASResetPasswordCallback)callBack;
{
  ...
  CTSRestCoreRequest* request = [[CTSRestCoreRequest alloc]
      initWithPath:MLC_RESET_PASSWORD_NEW_PATH  //MLC_REQUEST_CHANGE_PWD_REQ_PATH -> old
...
-(void)requestSetPassword:(NSString *)password userName:(NSString *)userName completionHandler:(ASSetPassword)callback{
    ...
    [self requestChangePasswordUserName:userName oldPassword:[self generateBigIntegerString:userName] newPassword:password completionHandler:^(NSString *response, NSError *error) {
...
- (void)requestChangePasswordUserName:(NSString*)userName
  ...
  [self addCallback:callback forRequestId:ChangePasswordReqId];
    ...
    if (oauthStatus.error != nil) {
        [self changePasswordHelper:nil withError:oauthStatus.error];
    }
    
    userName = userName.lowercaseString;
    
    if (![CTSUtility validateEmail:userName]) {
        [self changePasswordHelper:nil withError:[CTSError getErrorForCode:EmailNotValid]];
        return;
    }

    
  CTSRestCoreRequest* request = [[CTSRestCoreRequest alloc]
      initWithPath:MLC_CHANGE_PASSWORD_NEW_PATH   //MLC_CHANGE_PASSWORD_REQ_PATH -> old
...
- (BOOL)signOut {
  ...
  [CTSUtility removeLoginId];
  [CTSCVVEncryptor removeStoredCVV];
  return YES;
}
...
- (void)handleReqChangePassword:(CTSRestCoreResponse*)response {
    [self changePasswordHelper:response.responseString withError:response.error];
}
...
- (void)changePasswordHelper:(NSString *)response withError:(NSError*)error {
...
- (void)bindUserHelperUsername:(NSString *)userName error:(NSError*)error {
    ...
    if(error == nil){
        [CTSUtility storeLoginId:userName];
    }
