...
+ (NSString*)fetchCardSchemeForCardNumber:(NSString*)cardNumber {
    ...
    if ([CTSUtility hasPrefixArray:MAESTRO cardNumber:cardNumber]) {
        ...
        if ([CTSUtility hasPrefixArray:amex cardNumber:cardNumber]) {
            ...
            if ([CTSUtility hasPrefixArray:discover cardNumber:cardNumber]) {
                cardString = @"DISCOVER";
            }
            else
                if ([CTSUtility hasPrefixArray:JCB cardNumber:cardNumber]) {
                    cardString = @"JCB";
                }
                else
                    if ([CTSUtility hasPrefixArray:DinerClub cardNumber:cardNumber]) {
                        cardString = @"DINERCLUB";
                    }
                    else
                        if ([CTSUtility hasPrefixArray:VISA cardNumber:cardNumber]) {
                            cardString = @"VISA";
                        }
                        else
                            if ([CTSUtility hasPrefixArray:MASTER cardNumber:cardNumber]) {
                                cardString = @"MCRD";
                            }
    if (cardString==nil) {
...
+ (NSMutableArray*)fetchMappedCardSchemeForSaveCards:(NSArray*)cardsSchemeArray {
    ...
    for (NSString *scheme in cardsSchemeArray) {
        ...
        //        else if ([scheme isEqualToString:@"Rupay Card"]) {
        //            [schemeArray addObject:@"VISA"];
        //        }
    }
    return schemeArray;
...
+(NSString *)fetchSigninSecret:(CTSKeyStore *)keyStore{
    if(keyStore && keyStore.signinSecret){
...
+(NSString *)fetchSigUpSecret:(CTSKeyStore *)keyStore{
    if(keyStore && keyStore.signUpSecret){
...
+(NSString *)grantTypeFor:(PasswordType)type{
    ...
    if(type == PassworTypeOtp){
        grantType = MLC_SIGNIN_GRANT_TYPE_OTP;
    }
    else if (type == PassworTypePassword){
