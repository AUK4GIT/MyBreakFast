...
- (instancetype)initWithKeyStore:(CTSKeyStore *)keystoreArg{
    CTSProfileLayer *profile = [self init];
...
- (void)updateContactInformation:(CTSContactUpdate*)contactInfo
  ...
  [self addCallback:callback forRequestId:ProfileUpdateContactReqId];
    ...
    OauthStatus* oauthStatus = [CTSOauthManager fetchBindSigninTokenStatus:keystore];
...
- (void)requestUpdateCashoutBankAccount:(CTSCashoutBankAccount*)bankAccount
    ...
    OauthStatus* oauthStatus = [CTSOauthManager fetchPasswordSigninTokenStatus:keystore];
    NSString* oauthToken = oauthStatus.oauthToken;
...
- (void)handleProfileGetPayment:(CTSRestCoreResponse*)response {
  ...
  if (error == nil) {
    ...
    LogTrace(@"jsonError %@", jsonError);
      ...
      debitArray = [CTSUtility fetchMappedCardSchemeForSaveCards:[[NSSet setWithArray:debitArray] allObjects] ];
      creditArray = [CTSUtility fetchMappedCardSchemeForSaveCards:[[NSSet setWithArray:creditArray] allObjects] ];
      
      for (NSDictionary *dict in paymentDetails.paymentOptions) {
          
          if ([@"debit" isEqualToString: [dict valueForKey:@"type"]]){
              
              for (int count = 0; count < debitArray.count; count ++) {
                  shouldDelete= YES;
                  if ([[dict valueForKey:@"scheme"] caseInsensitiveCompare:[debitArray objectAtIndex:count]]== NSOrderedSame) {
                      shouldDelete = NO;
                      break;
                  }
              }
              ...
              for (int count = 0; count < creditArray.count; count ++) {
                  shouldDelete= YES;
                  if ([[dict valueForKey:@"scheme"] caseInsensitiveCompare:[creditArray objectAtIndex:count]]== NSOrderedSame) {
                      shouldDelete = NO;
                      break;
                  }
              }
      ...
      NSMutableArray *array = (NSMutableArray *)paymentDetails.paymentOptions;
...
-(void)requestPaymentModes{
    CTSPaymentLayer *paymentLayer = [[CTSPaymentLayer alloc]init];
    [paymentLayer requestMerchantPgSettings:VanityUrl withCompletionHandler:^(CTSPgSettings *pgSettings, NSError *error) {
        if(error){
            ...
            debitArray = pgSettings.debitCard;
            creditArray = pgSettings.creditCard;
        }
    }];
}
